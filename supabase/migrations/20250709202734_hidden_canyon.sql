/*
  # Fix round function error

  1. Changes
     - Fix the "function round(double precision, integer) does not exist" error
     - Explicitly cast double precision values to numeric before using round()
     - Update the get_admin_analytics function to use proper type casting
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS get_admin_analytics();

-- Recreate the function with proper numeric casting
...
BEGIN
    SELECT json_build_object(
        ...
        'user_growth', (
            SELECT json_build_object(
                ...
                'growth_rate', CASE 
                    ...
                    ELSE round(((SELECT COUNT(*) FROM user_profiles WHERE created_at >= date_trunc('month', NOW()))::numeric / NULLIF((SELECT COUNT(*) FROM user_profiles WHERE created_at >= date_trunc('month', NOW() - INTERVAL '1 month') AND created_at < date_trunc('month', NOW())), 0)::numeric - 1) * 100, 2)
                END
            )
        ),
        'engagement_stats', (
            SELECT json_build_object(
                'average_session_length', COALESCE(round(AVG(EXTRACT(EPOCH FROM (updated_at - created_at)) / 60)::numeric, 2), 0),
        ...
        'streak_stats', (
            SELECT json_build_object(
                'average_streak', COALESCE(round(AVG(current_streak)::numeric, 1), 0),
